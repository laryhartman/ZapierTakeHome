...
SELECT Active.Monthname,Active.Day, Active.ActiveUsers, NULLIF(Churn.ChurnUsers,0) AS ChurnUsers
FROM (
		...
		SELECT MONTHNAME(TU.date) AS MonthName
				,DATE(TU.date) AS Day
...
LEFT JOIN (
		...
		GROUP BY DATE(churnDate)
		) AS Churn ON Active.Day = Churn.Day
ORDER BY Active.Day;